...
function CIECAM02m1 (viewingConditions={}, correlates=defaultCorrelates, p = -0.4) {
	...
	var RGB_w = matrix.multiply(XYZ_to_CAT02, XYZ_w),
			...
			console.log( 'Inital white cone response: ', RGB_w);

	 		RGB_w = modifyConeResponse(RGB_w, RGB_b, RGB_proximal, p);

	var [Rw, Gw, Bw] = RGB_w;
	var [D_R, D_G, D_B] = RGB_w.map(v => D * XYZ_w[1] / v + 1 - D);
	console.log('Modifyed white cone response: ', RGB_w);


	var RGB_cw = [D_R*Rw, D_G*Gw, D_B*Bw],
	    RGB_aw = adaptedResponses(RGB_cw),
	    A_w = achromaticResponse(RGB_aw);

	console.log('RGB_cw: ', RGB_cw);
